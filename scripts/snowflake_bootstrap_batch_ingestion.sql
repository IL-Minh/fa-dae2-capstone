...
CREATE TABLE IF NOT EXISTS TRANSACTIONS_STREAMING_KAFKA (
    ...
    USER_ID STRING,  -- Changed from INTEGER to STRING for UUID support
    AMOUNT NUMBER(18,2),
    CURRENCY STRING,
    MERCHANT STRING,
    CATEGORY STRING,
...
-- 3. Table for user registration data from CSV files
...
-- 4. Permanent stage for CSV uploads (reused across batch runs)
...
-- 5. Permanent stage for user registration CSV uploads
CREATE STAGE IF NOT EXISTS STG_USER_REGISTRATIONS
  FILE_FORMAT = (
    TYPE = CSV
    FIELD_DELIMITER = ','
    SKIP_HEADER = 1
